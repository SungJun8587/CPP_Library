...
//***************************************************************************
// BaseArrayQueue.h: interface for the CBaseArrayQueue class.
//
//***************************************************************************

#ifndef __BASEARRAYQUEUE_H__
#define __BASEARRAYQUEUE_H__
...
//
template<class TYPE>
class CBaseArrayQueue
{
public:
	CBaseArrayQueue();
	CBaseArrayQueue(int nSize);
	CBaseArrayQueue(int nSize, int nGrowth);
	~CBaseArrayQueue();
...
template<class TYPE>
CBaseArrayQueue<TYPE>::CBaseArrayQueue()
{
	m_nSize = 0;
...
template<class TYPE>
CBaseArrayQueue<TYPE>::CBaseArrayQueue(int nSize)
{
	m_nSize = 0;
	m_nGrowth = GROWTH_DEFAULT;
	m_pData = NULL;
...
template<class TYPE>
CBaseArrayQueue<TYPE>::CBaseArrayQueue(int nSize, int nGrowth)
{
	m_nSize = 0;
	m_nGrowth = GROWTH_DEFAULT;
	m_pData = NULL;
...
//
template<class TYPE>
void CBaseArrayQueue<TYPE>::RemoveAll()
{
...
//
template<class TYPE>
inline int CBaseArrayQueue<TYPE>::GetDataSize(int nTop, int nEnd) const
{
...
//
template<class TYPE>
inline int CBaseArrayQueue<TYPE>::NextPos(int nPos)
{
	return ++nPos % m_nSize;
}
...
//
template<class TYPE>
inline void CBaseArrayQueue<TYPE>::Initialize(TYPE** pData, int nSize)
{
...
//
template<class TYPE>
void CBaseArrayQueue<TYPE>::SetSize(int nNewSize)
{
	int i;
...
//
template<class TYPE>
int CBaseArrayQueue<TYPE>::Put(TYPE Object)
{
...
//
template<class TYPE>
int CBaseArrayQueue<TYPE>::Get(TYPE& Object)
{
	if( GetDataSize() <= 0 ) return -1;

	memcpy(&Object, m_pData[m_nTop], sizeof(Object));

	delete m_pData[m_nTop];
	m_pData[m_nTop] = NULL;

	m_nTop = NextPos(m_nTop);

	return 0;
}

#endif // ndef __BASEARRAYQUEUE_H__
