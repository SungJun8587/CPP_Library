...
void CMySQLAsyncSrv::FlushRemainingTasks()
{
	...
	if( remainingCount > 0 )
	{
		...
		while( !tempQueue.empty() )
		{
			...
			// [4번 수정] 원래 이 호출이 if( it != _mapCommand.end() ) 블록 안쪽에만 있어서,
			// 핸들러를 찾지 못한(else) 경로는 outstanding 카운터를 영원히 감소시키지 못했다.
			// if/else 바깥으로 옮겨 두 경로 모두(= pAsyncRq가 유효한 모든 경우) 항상 호출되게 한다.
...
bool CMySQLAsyncSrv::Action()
{
	...
	while( !_bStopThread.load() )
	{
		...
		if( _mapCommand.end() == it )
		{
			...
			// [3번 수정] 이 요청은 더 이상 처리되지 않고 버려지는 경로이므로, Push() 이전에
			// 호출부가 걸어둔 AddOutstandingRequest()와 짝을 맞춰 감소시켜야 한다. 이 호출이
			// 없으면 미등록 callIdent가 들어올 때마다 outstanding 카운터가 영구히 어긋난다.
			SubOutstandingRequest();
			continue;
		}
