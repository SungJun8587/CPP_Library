...
//***************************************************************************
// @brief DB 서비스 하나(예: 멤버/게임 도메인 중 하나)의 비동기 ADO 처리 엔진.
// @details
...
//
// [호출부 영향] CAdoAsyncSrv::Instance()를 쓰던 기존 코드는 전부
...
public:
	...
	CAdoAsyncSrv(const CAdoAsyncSrv&) = delete;
	CAdoAsyncSrv& operator=(const CAdoAsyncSrv&) = delete;
	...
	//        (그건 도메인마다 별도 CAdoAsyncSrv 인스턴스를 두는 게 맞음).
	//***************************************************************************
	...
	//***************************************************************************
	CAdoConnPool* GetAdoConnPool();

	//***************************************************************************
	// @brief 샤딩이 필요한 도메인에서, id로 담당 샤드의 커넥션 풀을 반환합니다.
	//***************************************************************************
	CAdoConnPool* GetAdoConnPool(uint64 id);

private:
	// [캡슐화] 이 넷은 StartService()/소멸자 내부에서만 정해진 순서로
	// 호출되어야 하는 구현 세부사항이다 — 외부에서 직접 호출하면
	// lifecycle이 깨질 수 있다.
	bool InitAdo(const CVector<CDBNode>& dbNodeVec, const int32 nMaxThreadCnt);
	void StartIoThreads();
...
private:
	// [변경] CAdoConnPool** + 수동 new/delete[] 대신 vector<unique_ptr<>> —
	// 예외 안전성 확보(초기화 도중 실패해도 이미 만든 풀들이 자동 정리됨),
	// 수동 delete 누락 위험 제거.
	std::vector<std::unique_ptr<CAdoConnPool>>		_adoPools;
