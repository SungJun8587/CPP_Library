...
extern "C" {
	...
	// @param   없음
	// @return  uint64 - 전원 공급 이후 누적된 64비트 CPU 클록 사이클 수
...
public:
	...
	// @return EDX 레지스터 값
	//***************************************************************************
	const uint32& EDX() const {
		return regs[3];
	}

private:
	uint32 regs[4]; // 레지스터 배열 (EAX, EBX, ECX, EDX)
};
