...
public:
    ...
    //***************************************************************************
    explicit CLockFreeSlotStack(uint32 capacity)
        : _capacity(capacity)
        , _nextFree(capacity)
    {
    ...
    bool Pop(uint32& outIndex)
    {
        ...
        for( ;;)
        {
            const uint32 index = GetIndex(oldHead);
            ...
            const uint32 nextIndex = _nextFree[index].load(std::memory_order_relaxed);
    ...
    bool Push(uint32 index)
    {
        ...
        for( ;;)
        {
            const uint32 oldIndex = GetIndex(oldHead);

            _nextFree[index].store(oldIndex, std::memory_order_relaxed);

            const uint32 version = GetVersion(oldHead);
            const uint64 newHead = MakeNode(version + 1, index);
...
private:
    ...
    //***************************************************************************
    static constexpr uint32 kNullIndex = 0xFFFFFFFFu;
    ...
    //***************************************************************************
    static uint64 MakeNode(uint32 version, uint32 index) noexcept
    {
        return (static_cast<uint64>(version) << 32) |
    ...
    // @return 추출된 버전 값
    //***************************************************************************
    static uint32 GetVersion(uint64 node) noexcept
    {
        return static_cast<uint32>(node >> 32);
    }
    ...
    // @return 추출된 슬롯 인덱스 값
    //***************************************************************************
    static uint32 GetIndex(uint64 node) noexcept
    {
        return static_cast<uint32>(node);
    }

private:
    uint32                        _capacity{ 0 };     // 관리하는 전체 슬롯 용량
    CVector<std::atomic<uint32>>  _nextFree;          // 각 슬롯의 다음 빈 슬롯 인덱스 링크 배열
    alignas(8) std::atomic<uint64>_head{ 0 };         // 스택의 최상단 헤드 (버전 + 인덱스 패킹)
};
