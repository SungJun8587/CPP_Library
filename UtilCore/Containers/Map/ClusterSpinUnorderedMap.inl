...
//***************************************************************************
template<typename T1, typename T2, __int32 nClusterCnt, bool bInnerLock, typename Hash>
bool CClusterSpinUnorderedMap<T1, T2, nClusterCnt, bInnerLock, Hash>::InsertObject(const T1& key, T2 object)
{
	__int32 nClusterIdx = getClusterIdx(key);

	// RAII 가드: try_emplace()가 예외(Hash/비교 연산자, 노드 할당 실패 등)를
	// 던져도 스코프 종료 시 반드시 writeUnlock이 호출됩니다.
...
// @details T1이 무거운 타입(문자열 등)일 때 key 복사 없이 이동으로 노드를 구성합니다.
...
//***************************************************************************
template<typename T1, typename T2, __int32 nClusterCnt, bool bInnerLock, typename Hash>
T2 CClusterSpinUnorderedMap<T1, T2, nClusterCnt, bInnerLock, Hash>::FindObject(const T1& key)
{
	__int32 nClusterIdx = getClusterIdx(key);
	T2 object{};
...
//***************************************************************************
template<typename T1, typename T2, __int32 nClusterCnt, bool bInnerLock, typename Hash>
bool CClusterSpinUnorderedMap<T1, T2, nClusterCnt, bInnerLock, Hash>::FindObject(const T1& key, T2& object)
{
	__int32 nClusterIdx = getClusterIdx(key);
	bool nRet = false;
...
//***************************************************************************
template<typename T1, typename T2, __int32 nClusterCnt, bool bInnerLock, typename Hash>
bool CClusterSpinUnorderedMap<T1, T2, nClusterCnt, bInnerLock, Hash>::EraseObject(const T1& key)
{
	__int32 nClusterIdx = getClusterIdx(key);
	bool nRet = false;
