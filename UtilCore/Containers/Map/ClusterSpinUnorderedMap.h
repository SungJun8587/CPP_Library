...
//      최적화 컴파일러가 나눗셈 대신 비트 연산(key & (nClusterCnt - 1))으로 최적화할 가능성이 높습니다
...
public:
	...
	CClusterSpinUnorderedMap(void) = default;
	...
	CClusterSpinUnorderedMap(const CClusterSpinUnorderedMap&) = delete;
	CClusterSpinUnorderedMap& operator=(const CClusterSpinUnorderedMap&) = delete;
	CClusterSpinUnorderedMap(CClusterSpinUnorderedMap&&) = delete;
	CClusterSpinUnorderedMap& operator=(CClusterSpinUnorderedMap&&) = delete;
...
public:
	...
	// @details 사용 예:
	//          {
	//              typename CClusterSpinUnorderedMap::ReadLockGuard guard(*this, key, __FUNCTION__);
	//              auto& m = GetObjectMap(key);
	//              // m을 다루는 도중 예외가 나도 소멸자에서 반드시 ReadUnlock 호출
	//          }
	//***************************************************************************
	class ReadLockGuard
	{
	public:
		ReadLockGuard(CClusterSpinUnorderedMap& owner, const T1& key, const char* name = nullptr)
	...
	private:
		CClusterSpinUnorderedMap& m_owner;
		__int32 m_idx;
		const char* m_name;
	};
	...
	{
	public:
		WriteLockGuard(CClusterSpinUnorderedMap& owner, const T1& key, const char* name = nullptr)
			: m_owner(owner), m_idx(owner.getClusterIdx(key)), m_name(name)
		{
	...
	{
	public:
		ReadLockGuardByIdx(CClusterSpinUnorderedMap& owner, __int32 idx, const char* name = nullptr)
	...
	{
	public:
		WriteLockGuardByIdx(CClusterSpinUnorderedMap& owner, __int32 idx, const char* name = nullptr)
			: m_owner(owner), m_idx(idx), m_name(name)
		{
			m_owner.lockWrite(m_idx, m_name);
		}
...
protected:
	...
	//          해시, 비교 등)에서 예외가 발생해도 스코프 종료 시 항상 unlock이 보장됩니다.
	//          bInnerLock=false면 readLock/writeLock 자체가 no-op이므로 가드 비용도 없습니다.
	//***************************************************************************
	class InnerReadGuard
	{
	public:
		InnerReadGuard(CClusterSpinUnorderedMap& owner, __int32 idx, const char* name)
			: m_owner(owner), m_idx(idx), m_name(name)
		{
	...
	{
	public:
		InnerWriteGuard(CClusterSpinUnorderedMap& owner, __int32 idx, const char* name)
			: m_owner(owner), m_idx(idx), m_name(name)
		{
...
protected:
	...
	ObjectMap	m_ObjectMaps[nClusterCnt];	// 클러스터별로 데이터를 저장하는 해시맵 배열
