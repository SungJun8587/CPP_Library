...
bool CDBQueryProcess::GetDBSystemInfo(int32& iSystemCount, std::unique_ptr<DB_SYSTEM_INFO[]>& pDBSystemInfo)
{
	...
	_dbConn.BindCol(iSystemCount);

	if( _dbConn.Fetch() == false ) return false;

	if( iSystemCount == 0 ) return false;

	if( _dbConn.MoreResults() != SQL_SUCCESS ) return false;

	pDBSystemInfo = unique_ptr<DB_SYSTEM_INFO[]>(new DB_SYSTEM_INFO[iSystemCount]);

	_dbConn.FreeStmt();
	_dbConn.AllSets(sizeof(DB_SYSTEM_INFO), iSystemCount);
...
bool CDBQueryProcess::GetDatabaseList(int& iDBCount, std::unique_ptr<DB_INFO[]>& pDatabase)
{
	...
	if( _dbClass == EDBClass::MSSQL )
		query = _T("SELECT COUNT(*) AS [db_count] FROM sys.databases WHERE [name] NOT IN('model', 'msdb', 'pubs', 'Northwind', 'tempdb');");
	else if( _dbClass == EDBClass::MYSQL )
		query = _T("SELECT COUNT(*) AS `db_count` FROM INFORMATION_SCHEMA.SCHEMATA WHERE `SCHEMA_NAME` NOT IN('sys', 'mysql', 'information_schema', 'performance_schema', 'world');");
...
bool CDBQueryProcess::MSSQLGetRowStoreIndexFragmentationCheck(const TCHAR* ptszTableName, int32& iIndexCount, std::unique_ptr<MSSQL_INDEX_FRAGMENTATION[]>& pIndexFragmentation)
{
	...
	query = MSSQLGetRowStoreIndexFragmentationCheckQuery(StringToTString(ptszTableName), true) + _T(";");
	query = query + _T("\r\n") + MSSQLGetRowStoreIndexFragmentationCheckQuery(StringToTString(ptszTableName), false);
...
bool CDBQueryProcess::MSSQLGetColumnStoreIndexFragmentationCheck(const TCHAR* ptszTableName, int32& iIndexCount, std::unique_ptr<MSSQL_INDEX_FRAGMENTATION[]>& pIndexFragmentation)
{
	...
	query = MSSQLGetColumnStoreIndexFragmentationCheckQuery(StringToTString(ptszTableName), true) + _T(";");
	query = query + _T("\r\n") + MSSQLGetColumnStoreIndexFragmentationCheckQuery(StringToTString(ptszTableName), false);
...
bool CDBQueryProcess::MYSQLGetCharacterSets(const TCHAR* ptszCharset, int32& iCharsetCount, std::unique_ptr<MYSQL_CHARACTER_SET[]>& pCharacterSet)
{
	...
	if( ptszCharset != NULL && _tcslen(ptszCharset) > 0 )
		query = _T("SELECT COUNT(*) AS `charset_count` FROM INFORMATION_SCHEMA.CHARACTER_SETS WHERE `CHARACTER_SET_NAME` = '") + _tstring(ptszCharset) + _T("';");
	else query = _T("SELECT COUNT(*) AS `charset_count` FROM INFORMATION_SCHEMA.CHARACTER_SETS;");

	query = query + _T("\r\n") + MYSQLGetCharacterSetsQuery(StringToTString(ptszCharset));
...
bool CDBQueryProcess::MYSQLGetCollations(const TCHAR* ptszCharset, int32& iCharsetCount, std::unique_ptr<MYSQL_COLLATION[]>& pCollation)
{
	...
	if( ptszCharset != NULL && _tcslen(ptszCharset) > 0 )
		query = _T("SELECT COUNT(*) AS `charset_count` FROM INFORMATION_SCHEMA.COLLATIONS WHERE `CHARACTER_SET_NAME` = '") + _tstring(ptszCharset) + _T("';");
	else query = _T("SELECT COUNT(*) AS `charset_count` FROM INFORMATION_SCHEMA.COLLATIONS;");

	query = query + _T("\r\n") + MYSQLGetCollationsQuery(StringToTString(ptszCharset));
...
bool CDBQueryProcess::MYSQLGetCharacterSetCollations(const TCHAR* ptszCharset, int32& iCharsetCount, std::unique_ptr<MYSQL_CHARACTER_SET_COLLATION[]>& pCharacterSetCollation)
{
	...
	if( ptszCharset != NULL && _tcslen(ptszCharset) > 0 )
		query = _T("SELECT COUNT(*) AS `charset_count` FROM INFORMATION_SCHEMA.COLLATION_CHARACTER_SET_APPLICABILITY WHERE `CHARACTER_SET_NAME` = '") + _tstring(ptszCharset) + _T("';");
	else query = _T("SELECT COUNT(*) AS `charset_count` FROM INFORMATION_SCHEMA.COLLATION_CHARACTER_SET_APPLICABILITY;");

	query = query + _T("\r\n") + MYSQLGetCharacterSetCollationsQuery(StringToTString(ptszCharset));
...
bool CDBQueryProcess::MYSQLGetStorageEngines(int32& iStorageEngineCount, std::unique_ptr<MYSQL_STORAGE_ENGINE[]>& pStorageEngine)
{
	...
	query = _T("SELECT COUNT(*) AS `engine_count` FROM INFORMATION_SCHEMA.ENGINES;");
...
bool CDBQueryProcess::MYSQLGetTableFragmentationCheck(const TCHAR* ptszTableName, int32& iTableCount, std::unique_ptr<MYSQL_TABLE_FRAGMENTATION[]>& pTableFragmentation)
{
	...
	if( ptszTableName != NULL && _tcslen(ptszTableName) > 0 )
		query = _T("SELECT COUNT(*) AS `table_count` FROM INFORMATION_SCHEMA.TABLES WHERE `TABLE_SCHEMA` = DATABASE() AND `TABLE_NAME` = '") + _tstring(ptszTableName) + _T("';");
	else query = _T("SELECT COUNT(*) AS `table_count` FROM INFORMATION_SCHEMA.TABLES WHERE `TABLE_SCHEMA` = DATABASE();");

	query = query + _T("\r\n") + MYSQLGetTableFragmentationCheckQuery(StringToTString(ptszTableName));
