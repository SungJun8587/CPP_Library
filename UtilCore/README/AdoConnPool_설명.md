...
| False sharing 방지 | 슬롯별 원자 배열(`_pAdoConns`, `_pRefCount`, `_pReconnecting`, `_pNextRetryAllowedMs`, `_pRetryFailCount`)을 `CachePaddedAtomic<T>[]`로, `_slotLocks`를 캐시라인 정렬된 `SpinLockDefault[]`로 구성해 슬롯 간 캐시라인 공유를 차단 |
...
| `HealthCheckLoop()` | 격리 큐 청소 + 끊어진 슬롯(`GetDBCon()`이 거짓인 슬롯)을 스캔해 재연결 큐에 등록 (블로킹 I/O 없음) |
...
### 5.2 자동 재연결
1. `HealthCheckLoop()`이 500ms마다 순회하며 참조 카운트 0 & 연결 끊김(`GetDBCon()`이 거짓) & 백오프 통과한 슬롯을 찾음
...
- 풀 소멸 시 `~CAdoConnPool()`이 헬스체크/재연결 스레드를 먼저 종료한 뒤 `Clear()`로 자원을 정리한다.
